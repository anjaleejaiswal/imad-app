...
var articles={
 ...
 'article-one':{   title:'Article One|Anjalee Jaiswal',
    heading:'article one',
    date:' aug 8 ,2017',
    content: `
    <p>
...
'article-two':{title:  'Article Two |Anjalee Jaiswal',
    heading:'article two',
    date:' aug 9,2017',
    content: `
    <p>
...
'article-three':{title:'Article Three|Anjalee Jaiswal',
...
app.post('/create-user1',function(req,res){
  ...
  var salt=crypto.RandomBytes1(128).toString('hex');
...
app.get('/articles/:article2Name',function(req,res){
   
    pool.query("SELECT * FROM article1 WHERE title='"+req.params.article2Name+"'",function(err,result){
        if(err){
            res.status(500).send(err.toString());
            ...
            {
                res.status(404).send('Article2 not found');
                
            }else{
                var article2Data=result.rows[0];
                res.send(createTemplate (article2Data));
            }
        }
    });
